...
- Camera controls are **rotation only. Pan and zoom are both off.** Panning let people shove the whole grade off the edge of the screen with no way back; zoom let them scale it into a blur with no way back either. The graph is framed to fit and stays framed, orbit is the whole control set, and with nothing to pan or scale there is nothing to rubber-band at the edges. Leaving the wheel alone also means the landing page can scroll over a live map.
...
- **Backend/data**: Firebase — Auth (Google provider only), Firestore, Storage, Hosting. (Chosen over Supabase earlier: easier Google OAuth integration, live Firestore updates, more beginner tutorials.)
  - **Changed in Phase 4 (Sept 2026):** hosting moved to **GitHub Pages** and the backend to **Supabase** (Google sign-in through Supabase Auth, Postgres tables with row level security instead of Firestore rules, Supabase Storage instead of Firebase Storage). Everything below that says Firestore/Storage rules maps one-to-one onto an RLS policy on the matching Postgres table. The roadmap, data model and privacy rules are unchanged. See `README.md` and `supabase/migrations/` for the current setup.
  - **Changed in Phase 5 (Sept 2026):** the profile photo bucket is **private**, not public, because "never readable by the public internet" under Legal & privacy applies to faces too. Photos are read through short-lived signed URLs, so the `photoURL` field described below is stored as `people.photo_path` (an object path) and signed at read time.
